...
_GAME_FILES_CACHE = {}
_GAME_FILES_CACHE_VERSION = 1


def _cache_disabled():
    return os.environ.get("ALFWORLD_GAME_FILES_CACHE_DISABLE", "").lower() in {"1", "true", "yes"}


def _manifest_cache_path(cache_key):
    cache_dir = os.path.expandvars(
        os.environ.get("ALFWORLD_GAME_FILES_CACHE_DIR", "/tmp/skillrl-alfworld-cache")
    )
    os.makedirs(cache_dir, exist_ok=True)
    digest = hashlib.sha256(
        json.dumps(cache_key, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()[:24]
    return os.path.join(cache_dir, f"game_files_v{_GAME_FILES_CACHE_VERSION}_{digest}.json")


def _cache_key_payload(cache_key):
    return json.loads(json.dumps(cache_key))


def _load_manifest_cache(cache_key):
    if _cache_disabled():
        return None
    cache_path = _manifest_cache_path(cache_key)
    try:
        ...
        print(f"Failed to read ALFWorld game file cache {cache_path}: {exc}")
        return None

    if (
        payload.get("version") != _GAME_FILES_CACHE_VERSION
        or payload.get("cache_key") != _cache_key_payload(cache_key)
    ):
        return None
    game_files = payload.get("game_files")
    if not isinstance(game_files, list):
        return None
    print(f"Loaded {len(game_files)} ALFWorld game files from cache {cache_path}")
    return tuple(game_files)


def _write_manifest_cache(cache_key, game_files):
    if _cache_disabled():
        return
    cache_path = _manifest_cache_path(cache_key)
    ...
    payload = {
        ...
        "cache_key": _cache_key_payload(cache_key),
    ...
    try:
        ...
        print(f"Wrote {len(game_files)} ALFWorld game files to cache {cache_path}")
    except Exception as exc:
        print(f"Failed to write ALFWorld game file cache {cache_path}: {exc}")
    finally:
        if os.path.exists(tmp_path):
            os.remove(tmp_path)


def _collect_game_files_with_process_cache(cache_key, collect_fn):
    cached_game_files = _load_manifest_cache(cache_key)
    if cached_game_files is not None:
        return cached_game_files

    if _cache_disabled():
        game_files = collect_fn()
        return tuple(game_files)
    ...
    try:
        ...
        deadline = time.monotonic() + float(
            os.environ.get("ALFWORLD_GAME_FILES_CACHE_WAIT_SECONDS", "300")
        )
        ...
        if cached_game_files is not None:
            return cached_game_files
        game_files = collect_fn()
        _write_manifest_cache(cache_key, game_files)
        return tuple(game_files)
...
class AlfredTWEnv(object):
    ...
    def collect_game_files(self, verbose=False):
        ...
        cache_key = (self.train_eval, data_path, tuple(task_types))
        cached_game_files = _GAME_FILES_CACHE.get(cache_key)
        if cached_game_files is None:
            ...
            _GAME_FILES_CACHE[cache_key] = _collect_game_files_with_process_cache(cache_key, collect_game_files)
        self.game_files = list(_GAME_FILES_CACHE[cache_key])

        print(f"Overall we have {len(self.game_files)} games in split={self.train_eval}")
        self.num_games = len(self.game_files)
        if self.num_games == 0:
            raise RuntimeError(
                f"No ALFWorld games found for split={self.train_eval} under {data_path!r}. "
                "Check ALFWORLD_DATA and the json_2.1.1 dataset layout."
            )
